...
export function CourseAwareOnboardingProvider({
  children
}: {
  ...
  const { updateCourseProgress, getCourseProgress } = useCourseProgress();

  // Load completed steps from localStorage and sync with course progress
  useEffect(() => {
  ...
  const nextStep = () => {
    ...
    if (currentIndexInSection < sectionSteps.length - 1) {
      ...
      const completedCount = currentIndexInSection + 1;
      const progress = Math.round((completedCount / sectionSteps.length) * 100);
      updateCourseProgress(
        `physician-analytics-${currentSection}`,
        progress,
        completedCount,
        sectionSteps.length
      );
  ...
  const getSectionProgress = (section: string) => {
    const courseProgress = getCourseProgress(`physician-analytics-${section}`);
