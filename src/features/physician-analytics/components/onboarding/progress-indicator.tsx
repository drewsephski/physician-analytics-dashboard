...
export function ProgressIndicator({ className = '' }: ProgressIndicatorProps) {
  const { isStepCompleted, getStepsForSection, startOnboarding } =
    useOnboarding();
    ...
    return {
      ...
      percentage:
        sectionSteps.length > 0
          ...
          : 0,
      isComplete:
        completedCount === sectionSteps.length && sectionSteps.length > 0
    };
  };

  const overallStatus = getCompletionStatus();
  const isFullyComplete = overallStatus.percentage === 100;
            ...
            <>
              <div className='relative'>
                ...
                <div
                  ...
                  style={{
                    clipPath: `polygon(0 0, ${overallStatus.percentage}% 0, ${overallStatus.percentage}% 100%, 0 100%)`
                  }}
                />
              </div>
              <span className='text-sm'>
                {Math.round(overallStatus.percentage)}% Complete
              </span>
            </>
          )}
        </Button>
          ...
          <div className='space-y-2'>
            <div className='flex items-center justify-between text-sm'>
              ...
              <span className='font-medium'>
                {overallStatus.completed}/{overallStatus.total} steps
              </span>
            </div>
            <Progress value={overallStatus.percentage} className='h-2' />
          </div>
              ...
              <div className='flex items-start gap-2'>
                ...
                <div className='text-xs'>
                  ...
                  <p className='leading-relaxed text-green-700 dark:text-green-400'>
                    You&apos;ve completed all training modules. You&apos;re now
                    ready to maximize the platform&apos;s potential for hospital
